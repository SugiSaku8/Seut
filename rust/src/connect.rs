...
pub async fn connect(data: &HashMap<String, String>, port: u16) -> Result<ResponseData, Error> {
    ...
    if !response.status().is_success() {
        ...
        return Err(reqwest::Error::from(std::io::Error::new(std::io::ErrorKind::Other, error_message)));
    }
    ...
    let response_data: ResponseData = serde_json::from_str(&response_body).map_err(reqwest::Error::from)?;
