...
const DC = class {
  ...
  async ordar(cmd, config) {
    ...
    if (typeof m0 === "undefined" || m0 === "undefined") {
      console.error(
        "Insufficient memory capacity. \nClear the memory in the specified manner."
      );
      init(ram, "memory", SIZE);
    }
    let m1 = get_ntn_blank(memory);
    console.log("Next-Priority free memory ID:" + m1);
    if (typeof m1 === "undefined"|| m1 === "undefined") {
      console.error(
        "Insufficient memory capacity. \nClear the memory in the specified manner."
      );
      init(ram, "memory", SIZE);
    ...
    if (
      ...
      if (typeof cmd.cmd == "undefined") {
        ...
        console.error(
          "Of course. If you don't specify an order, the command post won't do anything for you, okay?"
        );
      }
    }
  }
