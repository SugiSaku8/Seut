...
const DC = class{
    ...
    ordar(cmd,DCclass,config){
                ...
                switch(config.network.type){
                    case "cell":
                        switch(v.check(cmd.command.raw)){
                            case true:
                                console.warn("The defenses of the cell network detected that the specified command was suspicious.\nthis is supercalifragilisticexpialidocious!")
                                return {'status':false,'log':'Detected by cell network defense function'};
                            }
                    default:
                        break;
                }
        ...
        if(cmd.name!=undefined&&cmd.sender!=undefined&&config.version!=undefined&&cmd.time!=undefined&&cmd.cmd.command!=undefined){
            let m0_obj = {
                "name":cmd.name,
                "sender":cmd.sender,
